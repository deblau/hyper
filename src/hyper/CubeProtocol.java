...
public class CubeProtocol
{
	...
	private static HashMap<CubeMessageType, CubeMessageType> sm = new HashMap<CubeMessageType, CubeMessageType>() {
		{
			// INN transitions
			put(CubeMessageType.CONN_GEN_INN_AVAIL, CubeMessageType.CONN_INN_GEN_ANN);
			put(CubeMessageType.CONN_ANN_INN_SUCCESS, CubeMessageType.CONN_INN_ANN_HANDOFF);
			put(CubeMessageType.CONN_ANN_INN_FAIL, CubeMessageType.CONN_INN_ANN_HANDOFF);

			// ANN transitions
			put(CubeMessageType.CONN_INN_ANN_HANDOFF, CubeMessageType.CONN_GEN_INN_AVAIL);
			put(CubeMessageType.CONN_EXT_ANN_ACCEPT, CubeMessageType.CONN_ANN_EXT_OFFER);
			put(CubeMessageType.CONN_EXT_ANN_DECLINE, CubeMessageType.CONN_ANN_EXT_OFFER);
			put(CubeMessageType.CONN_NBR_ANN_CONNECTED, CubeMessageType.CONN_ANN_NBR_CONNECT);
			put(CubeMessageType.CONN_NBR_ANN_DISCONNECTED, CubeMessageType.CONN_ANN_NBR_CONNECT);
			put(CubeMessageType.CONN_NBR_ANN_IDENTIFIED, CubeMessageType.CONN_ANN_NBR_IDENTIFY);
			put(CubeMessageType.CONN_INN_GEN_CLEANUP, CubeMessageType.CONN_GEN_INN_AVAIL);

			// NBR transitions
			put(CubeMessageType.CONN_EXT_NBR_ACCEPT, CubeMessageType.CONN_NBR_EXT_OFFER);
			put(CubeMessageType.CONN_EXT_NBR_DECLINE, CubeMessageType.CONN_NBR_EXT_OFFER);
			put(CubeMessageType.CONN_ANN_NBR_IDENTIFY, CubeMessageType.CONN_NBR_ANN_CONNECTED);

			// EXT transitions
			put(CubeMessageType.CONN_ANN_EXT_OFFER, CubeMessageType.CONN_EXT_INN_ATTACH);
			put(CubeMessageType.CONN_NBR_EXT_OFFER, CubeMessageType.CONN_EXT_ANN_ACCEPT);
			put(CubeMessageType.CONN_NBR_EXT_IDENTIFY, CubeMessageType.CONN_EXT_ANN_ACCEPT);
			put(CubeMessageType.CONN_ANN_EXT_SUCCESS, CubeMessageType.CONN_EXT_ANN_ACCEPT);
			put(CubeMessageType.CONN_ANN_EXT_FAIL, CubeMessageType.CONN_EXT_ANN_ACCEPT);
		}
	};
	...
	private boolean processLocally(CubeMessage msg)
	{
		CubeAddress none = CubeAddress.INVALID_ADDRESS;
		CubeAddress rvrs = CubeAddress.BCAST_REVERSE;
		CubeAddress proc = CubeAddress.BCAST_PROCESS;

		// Determine the destination
		CubeAddress src = msg.getSrc();
		CubeAddress dst = msg.getDst();

		// Quickly bail based on source or destination, if we can
		if (null == src || none.equals(src) || rvrs.equals(src) || null == dst || none.equals(dst) || src.equals(dst))
			return true;
		...
		// Broadcast processing. If we are at the end of the broadcast chain, return immediately
		if (msg.getTravel().equals(BigInteger.ZERO))
			return true;

		// Forward the message, and return based on whether we should (also) process the message
		bcastSend(msg);
		return dst.equals(proc);
	}
	...
	void process(CubeMessage msg)
	{
		...
		// Ensure the message has the proper format
		if (!msg.hasProperFormat()) {
			msg.replyInvalid(CubeMessageType.INVALID_FORMAT);
			return;
		}

		// Forward messages that are not meant for me
		if (!processLocally(msg))
			return;
	...
	private void conn_inn_gen_ann(CubeMessage msg)
	{
		...
		if (!willing)
			payload = new BigInteger[] { zero.setBit(cubeState.addr.intValue()), zero };
		else if (cubeState.vacancy())
			payload = new BigInteger[] { zero, zero.setBit(cubeState.addr.intValue()) };
		else
			payload = new BigInteger[] { zero, zero };
		...
		if (!willing)
			...
			annState.state = CubeMessageType.CONN_GEN_INN_AVAIL;
	}
	...
	private void conn_inn_ann_handoff(CubeMessage msg)
	{
		...
		new CubeMessage(CubeAddress.INVALID_ADDRESS, annState.peerAddr, annState.state, null, cubeState.getDim())
	...
	private void connectNodeToINN(InetSocketAddress addr, SocketChannel innChan)
	{
		...
		if (!amWilling(addr)) {
			new CubeMessage(none, none, CubeMessageType.CONN_INN_EXT_CONN_REFUSED, null, null).send(innChan);
			Utilities.quietClose(innChan);
			return;
		}
		...
		new CubeMessage(none, peerAddr, CubeMessageType.CONN_ANN_EXT_OFFER, null, cubeState.getDim()).send(annChan);
		try {
		...
		new CubeMessage(cubeState.addr, peerAddr, CubeMessageType.CONN_NBR_EXT_IDENTIFY, null, null).send(annChan);
		try {
		...
		bcastSend(new CubeMessage(cubeState.addr, CubeAddress.BCAST_PROCESS, CubeMessageType.CONN_INN_GEN_CLEANUP, addr,
				null));
		new CubeMessage(cubeState.addr, peerAddr, CubeMessageType.CONN_ANN_EXT_SUCCESS, null, null).send(annChan);
	...
	private void conn_ann_ext_offer(CubeMessage msg)
	{
		...
		// Utilities.quietClose(cltState.annChan);
		// return;
		// }
		...
		InetSocketAddress addr = Utilities.quietRemote(cltState.annChan);
		...
		new CubeMessage(cubeState.addr, none, cltState.state, null, null).send(cltState.annChan);
	}
	...
	private void conn_ext_ann_accept(CubeMessage msg)
	{
		...
		//
		// // Validate the source and destination
		// InetSocketAddress addr = Utilities.quietAddr(chan);
		// ANNState annState = annStates.get(addr);
		// CubeAddress none = CubeAddress.INVALID_ADDRESS;
		...
		SocketChannel chan = msg.getChannel();
		InetSocketAddress addr = Utilities.quietRemote(chan);
		CxnState annState = cxnStates.get(addr);
		...
		if (cubeState.getDim() > 1 + annState.invalid.bitCount()) {
			...
			annState.replies = 1;
			annState.state = CubeMessageType.CONN_ANN_NBR_CONNECT;
			annBcast(annState, addr);
			return;
		}

		// Edge case: I'm the only neighbor connected, skip straight to Phase 4
		new CubeMessage(cubeState.addr, annState.peerAddr, CubeMessageType.CONN_NBR_EXT_IDENTIFY, null, null)
				.send(chan);
		new CubeMessage(cubeState.addr, annState.peerAddr, CubeMessageType.CONN_ANN_EXT_SUCCESS, null, null).send(chan);
	...
	private void conn_ext_ann_decline(CubeMessage msg)
	{
		...
		InetSocketAddress addr = Utilities.quietRemote(msg.getChannel());
		Utilities.quietClose(msg.getChannel());
		annBail(addr);
	}
	...
	private void conn_ann_nbr_connect(CubeMessage msg)
	{
		...
		new CubeMessage(none, none, nbrState.state, null, null).send(nbrState.nbrChan);
		try {
	...
	private void conn_nbr_ext_offer(CubeMessage msg)
	{
		...
		if (amWilling(addr)) {
			...
			new CubeMessage(cubeState.addr, none, CubeMessageType.CONN_EXT_NBR_ACCEPT, null, null).send(chan);
		} else {
			...
			new CubeMessage(cubeState.addr, none, cltState.state, Utilities.quietLocal(chan), null).send(chan);
			for (SocketChannel c : cubeState.neighbors)
				Utilities.quietClose(c);
	...
	private void conn_nbr_ann_connected(CubeMessage msg)
	{
		...
		// Enter Phase 4
		annState.replies = 1;
		annState.state = CubeMessageType.CONN_ANN_NBR_IDENTIFY;
		annBcast(annState, addr);

		annState.peerAddr = annState.peerAddr;
		annState.state = CubeMessageType.CONN_NBR_EXT_IDENTIFY;
		new CubeMessage(cubeState.addr, annState.peerAddr, annState.state, null, null);
	}
	...
	private void conn_nbr_ann_disconnected(CubeMessage msg)
	{
		...
		new CubeMessage(CubeAddress.INVALID_ADDRESS, annState.peerAddr, annState.state, null, null).send(chan);
		Utilities.quietClose(chan);
	...
	private void conn_ann_nbr_identify(CubeMessage msg)
	{
		...
		new CubeMessage(cubeState.addr, nbrState.peerAddr, nbrState.state, null, null).send(nbrState.nbrChan);
	...
	private void conn_nbr_ext_identify(CubeMessage msg)
	{
		...
		if (-1 == link) {
			new CubeMessage(cubeState.addr, none, CubeMessageType.INVALID_DATA, Utilities.quietLocal(chan),
	...
	private void conn_nbr_ann_identified(CubeMessage msg)
	{
		...
		new CubeMessage(cubeState.addr, annState.peerAddr, CubeMessageType.CONN_NBR_EXT_IDENTIFY, null, null)
				.send(annState.annChan);
		new CubeMessage(cubeState.addr, annState.peerAddr, CubeMessageType.CONN_ANN_EXT_SUCCESS, null, null)
				.send(annState.annChan);
	...
	private void conn_ann_inn_success(CubeMessage msg)
	{
		...
		Utilities.quietClose(innState.innChan);

		// Inform the other ANNs that they can flush state
		innState.state = CubeMessageType.CONN_INN_GEN_CLEANUP;
		bcastSend(new CubeMessage(cubeState.addr, CubeAddress.BCAST_PROCESS, innState.state, addr, null));
	}
	...
	private void conn_ann_nbr_fail(CubeMessage msg)
	{
		...
		if (nbrState.nbrChan.isOpen())
			Utilities.quietClose(nbrState.nbrChan);
	}
	...
	private void invalid_address(CubeMessage msg)
	{
		...
		if (origType == CubeMessageType.UNICAST_MSG) {
			...
			queued.add(new CubeMessage(CubeAddress.INVALID_ADDRESS, msg.getDst(), CubeMessageType.INVALID_FORMAT,
	...
	private void annBail(InetSocketAddress addr)
	{
		...
		annState.state = CubeMessageType.CONN_ANN_NBR_FAIL;
		annBcast(annState, addr);
